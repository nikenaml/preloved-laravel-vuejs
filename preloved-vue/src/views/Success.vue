<template>
  <div>
    <div class="d-flex success-checkout align-items-center justify-content-center">
      <div class="col col-lg-4 text-center">
        <img src="img/success-buy.png" alt width="294" />
        <h3 class="mt-4">Sukses Pemesanan!</h3>
        <p
          class="mt-2"
        >Silakan tunggu update terbaru proses pemesanan Anda dari kami melalui via email yang sudah Anda daftarkan sebelumnya.</p>
        <router-link to="/">
          <a href="index.html" class="primary-btn pd-cart mt-3">Back to Home</a>
        </router-link>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "Success",
  components: {}
};
</script>

<style scoped>
</style>